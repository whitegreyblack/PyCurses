...
class MainScreen:
    ...
    def check(self, y, x):
        if self.world[y][x] == -1:
            self.screen.addstr(6, 5, f"SCR: {self.__class__.__name__}")
        else:
...
class Screen:
    ...
    def translate(self, y, x):
        ...
        self.screen.addstr(2,2,f"{y - wy} {x - wx}")
        self.screen.addstr(3,3,f"{self.toggle}")
...
def main(scr):
    ...
    while c != ord('q'):
        ...
        if c == curses.KEY_MOUSE:
            ...
            mc.screen.addstr(5, 5, f"X:{x} Y:{y}")
            mc.check(y, x)

        for i in range(30):
            for j in range(30):
                mc.screen.addch(i, j + 30, f"{mc.world[i][j] + 1}")
