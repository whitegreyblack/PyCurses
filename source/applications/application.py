...
class Application(Loggable):
    ...
    """
    def __init__(self, folder, term=None, logger=None):
    ...
    def run(self):
        ...
        while self.running:
            ...
            handled = self.window.handle_key(key)
            if not handled:
                continue
            self.window.draw()
    ...
    #         screen.subwin(height - 2, utils.partition(width, 3), 1, 0),
    #         title="receipt",
    #         title_centered=True,
    #         focused=True,
    ...
    def build_application(self, rebuild=False, reinsert=False, examples=False):
        ...
        # sub1 = DisplayWindow(
        #     screen.subwin(height - 1, utils.partition(width, 2), 0, 0),
        #     title="Sub-win 1",
        #     dataobj=LabelList(
        #         Label.random(),
        #         Label.random(),
        ...
        # sub2 = DisplayWindow(
        #     screen.subwin(
        #         utils.partition(height-1, 2, op=math.ceil),
        #         utils.partition(width, 2), 
        #         0,
        #         utils.partition(width, 2), 
        #     ),
        #     title="Sub-win 2",
